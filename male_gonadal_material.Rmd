---
title: "Integrate male in house and public embryo gonad datasts"
output:
  html_document:
    keep_md: true
    smart: false
...
* J. Guo testis dataset: 10.1016/j.stem.2020.12.004
* Male in house sequenced datasets, as described in: 
  * https://git.lumc.nl/sasc/project-284-scRNA_gonads/-/blob/master/README.md 
  * Use batch 0 and Batch 2  
...
## Load data
```{r}
G3_data <- Read10X_h5(filename = "/path/to/G3/filtered_feature_bc_matrix.h5")
G4_data <- Read10X_h5(filename = "/path/to/G4/filtered_feature_bc_matrix.h5")
```
...
G3[["trimester"]] <- "1st" 
G3[["sex"]] <- "male" 
G3[["sample_id"]] <- "MG1+LT1"  
G3[["batch"]] <- "batch_0"
G3[["tissue"]] <- "G3" 
G3[["capture"]] <- "batch_0_G3" 

cell_id2calledS <- read.table(file = "/path/to/gonads_cell_id2calledS.tsv",
                              header = TRUE, 
                              sep = "\t")
cell_id2calledS$sample_id <- sapply(strsplit(x = cell_id2calledS$cell_bc, split = ":", fixed = TRUE), function(x) x[[1]])
cell_id2calledS$clean_barcode <- sapply(strsplit(x = cell_id2calledS$cell_bc, split = ":", fixed = TRUE), function(x) x[[2]])
G3_cellid_2_barcode <- cell_id2calledS %>% filter(sample_id=="G3")
calledS <- G3_cellid_2_barcode$calledS
names(calledS) <- G3_cellid_2_barcode$clean_barcode
G3 <- AddMetaData(object = G3, 
                           metadata = calledS, 
                           col.name = "scsm")
G3 <- subset(G3, scsm != "doublet")
table(G3[["scsm"]])
age <- G3[["scsm"]]$scsm
age[age == "SID04"] <- "wpf_09_a"
age[age == "SID05"] <- "wpf_09_b"
G3[["age"]] <- age
...
* Add metadata
```{r}
batch_2 <- readRDS("/path_to/hash_vs_scsm/batch_2_jasin.rds")
...
```{r}
meso <- readRDS("/path/to/meso_b.rds")
...
age[age == "TK1"] <- "wpf_07_c"
age[age == "TN1"] <- "wpf_11_a"
age[age == "TR5"] <- "wpf_17_b"
meso[["age"]] <- age
tissue <- meso[['sample_id']][,"sample_id"]
tissue[tissue == "TK1"] <- "gonads+meso"
tissue[tissue == "TN1"] <- "meso"
tissue[tissue == "TR5"] <- "meso"
meso[["tissue"]] <- tissue
trimester <- meso[['sample_id']][,"sample_id"]
trimester[trimester == "TK1"] <- "1st"
trimester[trimester == "TN1"] <- "1st"
trimester[trimester == "TR5"] <- "2nd"
meso[["trimester"]] <- trimester
```

## Merge all male objects into one
```{r}
object <- merge(x = batch_0,
                y = c(batch_2, meso))
Idents(object) <- "male"
rm(guo_testis, batch_2, batch_0, meso)
```
...
saveRDS(object = object, file = "male_gonadal.rds")
```
...
DimPlot(object,
       reduction = "umap",
       group.by = "dbl_finder_class")
saveRDS(object = object, file = "male_gonadal_mnn.rds")
```
...
DimPlot(object,
       reduction = "umap",
       group.by = "dbl_finder_class")
saveRDS(object = object, file = "male_gonadal_remove_doublets_mnn.rds")
SaveH5Seurat(object, filename = "male_gonadal_remove_doublets_mnn.h5Seurat")
Convert("male_gonadal_remove_doublets_mnn.h5Seurat", dest = "h5ad")
cell_bc <- colnames(object)
write.table(x = cell_bc, 
            file = "cell_barcodes_male_gonadal_remove_doublets_mnn.tsv", 
            row.names = FALSE,
            quote = FALSE)
```
...
write.table(x = filtered_markers,
            file = paste0(outputDir, "/filtered_markers_remove_doublets_mnn.tsv"),
            row.names = FALSE)
...
gprofilerPath = paste0(outputDir, "/gprofiler_degs_remove_doublets_mnn.tsv")
